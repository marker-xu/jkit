...
class JKit_Log extends Kohana_Log {
	...
	protected $_arrWriterNameMap = array();
	 ...
	 * 删除一个writer
	 * 
	 * @param object|string $writer 可以是writer对象或者attach时指定的名称
	 * @see Kohana_Log::detach()
	 */
	public function detach($writer) {
	    if (! is_object($writer)) {
	        if (! isset($this->_arrWriterNameMap[$writer])) {
	            return;
	        }
	        $objWriter = $this->_arrWriterNameMap[$writer];
	        unset($this->_arrWriterNameMap[$writer]);
	    } else {
	        $objWriter = $writer;
	    }
	    return parent::detach($objWriter);	    
	}
	 ...
	 * @param int $min_level
	 * @param string $strName 该writer的唯一标识，可以在detach()中使用
	 * @see Kohana_Log::attach()
	 */
	public function attach(Log_Writer $writer, $levels = array(), $min_level = 0, $strName = NULL) {
	    parent::attach($writer, $levels, $min_level);
	    if ($strName === NULL) {
	        $strName = "{$writer}";
	    }
	    $this->_arrWriterNameMap[$strName] = $writer;
