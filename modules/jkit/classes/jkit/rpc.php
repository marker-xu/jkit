...
class JKit_Rpc {
	 ...
	 */
	public static function call($strServerName, $mixedAction, $arrInput = array(), $intRetry = 2) {
		...
		switch ($arrConf['type']) {
			...
			case RPC_TYPE_THRIFT:
				...
				if (isset($arrInput['balance_key'])) {
				    $arrCallParam = array('action' => $mixedAction, 'balance_key' => $arrInput['balance_key']);
				    unset($arrInput['balance_key']);
				    $arrCallParam['args'] = $arrInput;
				} else {
