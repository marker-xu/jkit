...
abstract class JKit_Rpc_Abstract {
    ...
    protected $_bolSetBalanceKey = false;
    ...
    public function call($arrInput, $intRetry = 1) {
        if (isset($arrInput['balance_key'])) {
            if (is_int($arrInput['balance_key'])) {
                $this->_intBalanceKey = $arrInput['balance_key'];
            } else {
                $this->_intBalanceKey = crc32($arrInput['balance_key']);
            }
            $this->_bolSetBalanceKey = true;
        }
    }
    /**
    ...
    public function connect($intConnectRetry = 1) {
		...
		if ( $this->_bolSetBalanceKey && $this->_intBalanceKey!==NULL ) {
		    $objBalance->setBalanceKey($this->_intBalanceKey);
		    $this->_bolSetBalanceKey = false;
		}
