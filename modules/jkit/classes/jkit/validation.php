...
class ValidAdaptor{
	...
	public function datetype($datatype, $rule, $field){
		...
		switch($datatype){
			...
			case 'cnname'			:	return array(array('regex', array(':value', '/^[\x{4e00}-\x{9fa5}a-zA-Z.\x{3002}\x{2022}]{2,32}$/u'))); //中文姓名
			...
			case 'reg'				:	$_rule = preg_match("/\\\u([0-9a-f]{4})/i", $rule['reg-pattern']) ? 
											preg_replace("/(\/)?([^\/]+)(\/([a-z]{0,5}))?/i", '/$2/$4u', $rule['reg-pattern']):$rule['reg-pattern'];
										$_rule = preg_replace("/\\\u([0-9a-f]{4})/iU",'\x{$1}',$_rule);
										
										$ret = array(array('regex', array(':value', $_rule))); //正则验证---需要配有reg-pattern属性	
