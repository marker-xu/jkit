...
abstract class JKit_Controller extends Kohana_Controller{
	...
	function before(){
		parent::before();
        $strTmpDirectory = $this->request->directory(); 
        $strTemplateName = str_replace('_', '/', $this->request->controller()) . '/' . $this->request->action();
        if( $strTmpDirectory ) {
            $strTemplateName = $strTmpDirectory. '/' . $strTemplateName;
        }
		self::$_template = View::factory( $strTemplateName );
	...
	public function __call($name, $args) {
	    $strTplFile = str_replace('_', '/', $this->request->controller()) . '/' . $this->request->action();
	    if (Kohana::find_file('views', $strTplFile)) {
