...
class Kohana_Kodoc_Markdown extends MarkdownExtra_Parser {
	...
	{
		//当模板引用被pre或code转义的时候，应该不输出才对
		if(method_exists('HTML', 'clean')){
			$text2 = HTML::clean($text, array('HTML.ForbiddenElements'=>'code,pre', 
				'Core.HiddenElements' => array('code'=>true, 'pre'=>true)));
		}else{
			$text2 = $text;
		}

		if (preg_match_all('/{{([^\s{}]++)}}/', $text2, $matches, PREG_SET_ORDER))
		{
			$replace = array();
