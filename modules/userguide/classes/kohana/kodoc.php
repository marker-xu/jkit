...
class Kohana_Kodoc {
	...
	public static function classes(array $list = NULL)
	{
		...
		foreach ($list as $name => $path)
		{
			...
			{
				$path_filters = Kohana::$config->load('userguide.class_path_filter');

				foreach($path_filters as $filter){
					//filtering the path
	...
	{
		//name filter
		$prefix_filters = Kohana::$config->load('userguide.class_prefix_filter');
	
		foreach($prefix_filters as $filter){
			if (strpos($class->class->name, $filter) === 0)
			{
				return FALSE;
			}
		}
